...
APP_PATHS = {
    "Linux": [
        ...
        "/opt/Signal/resources/app.asar",  # Installation via .deb pkg
    ],
    "Windows": [os.getenv("LOCALAPPDATA", ""), r"Programs\signal-desktop\resources\app.asar"],
}

# returns path of signal-desktop
def app_path():
    try:
        paths = APP_PATHS[platform.system()]
        for path in paths:
        ...
        raise OSError(f"Is Signal installed ? looked for it in {paths}")
